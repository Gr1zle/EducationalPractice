...
function Cart() {
  const { cart, removeFromCart, clearCart, getTotalPrice, addPurchaseStats } = useCart();
  const navigate = useNavigate();

  const handleCheckout = () => {
    const totalAmount = getTotalPrice();
    if (totalAmount > 0) {
      addPurchaseStats(totalAmount);
      alert(`Заказ успешно оформлен! Сумма: ${totalAmount} V-баксов`);
      clearCart();
    }
  };
  ...
  return (
    <div className="main-content">
      ...
      <div className="cart-items" style={{width: '100%', maxWidth: '800px', marginBottom: '2rem'}}>
        {cart.items.map(item => (
          <div key={`${item.id}-${item.quantity}`} className="card" style={{
            display: 'flex', 
            alignItems: 'center', 
            gap: '1rem',
            flexDirection: 'row',
